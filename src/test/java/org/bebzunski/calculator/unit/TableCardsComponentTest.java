...
import org.bebzunski.calculator.services.TableCardsComponent;
...
class TableCardsComponentTest {
	...
	void dealTimes_whenDidNotSelectStreets_expectedFiveDifferentCards() {
		...
		// when
		List<TableCardsIteration> outputs = dealOneTime(tableCardsComponent);

		// then
		assertThat(outputs).hasSize(1);
		TableCardsIteration output = outputs.getFirst();
	...
	void dealTimes_whenAllStreetsSelected_thenOnlyOneOutput() {
		...
		// when
		List<TableCardsIteration> outputs = tableCardsComponent.dealTimes(100);
	...
	void dealTimes_whenNotAllStreetsSelected_thenNrOfOutputsAsGiven() {
		...
		// when
		List<TableCardsIteration> outputs = tableCardsComponent.dealTimes(times);
	...
	void dealTimes_whenSelectedAllFourAcesOnFlopAndTurn_thenNoAcesOnRiver() {
		...
		// then
		assertThat(outputs).hasSize(times);
		Set<Face> faces = outputs.stream().map(TableCardsIteration::river).map(Card::face).collect(Collectors.toSet());
	...
	private List<TableCardsIteration> dealOneTime(TableCardsComponent tableCardsComponent) {
